...
**在使用本服务前，请您认真阅读，并充分理解本协议全部内容，特别是以加粗等方式显著标识的条款**。若您未满 14 周岁，请在法定监护人**单独同意**并在其监督下使用本服务。  

部分特定功能可能附有单独的使用规则或协议（以下简称“具体协议”，包括但不限于隐私政策、数据处理说明、功能使用须知等）。本协议与具体协议不一致时，以具体协议为准；其他情形下，二者与相关业务规则共同构成本服务的完整协议并具同等效力。

在您点击确认/勾选同意/实际使用本服务（包括但不限于访问页面、发起查询、生成或下载内容）之任一时点，即视为您已充分阅读并同意本协议的**全部条款**。**如您不同意本协议任一条款，请立即停止访问或使用本服务**。

本服务的服务器及数据处理均位于中华人民共和国境内。如您从其他国家或地区访问本服务，您有责任确保自身行为符合当地法律法规。同时，您同意本协议的解释与争议解决适用中华人民共和国大陆地区法律（详见第8.1条），并服从相关司法管辖。

#### 2. 服务描述
 2.1 本服务是一个**免费、非官方**开发的网页工具。其功能包括基于您个人游戏数据的成绩分析，以及通用的部分游戏信息展示。部分服务如下：
...
2.2 为支持“联合查分API”登录方式，本服务将在服务内部，向以下第三方地址发起数据请求：https://phib19.top:8080
...
这些数据不会以明文形式写入硬盘或数据库，并会在自缓存创建时起计24小时内自动过期清除。

您理解并同意，此缓存机制是为了优化服务响应速度而设计，一旦缓存过期缓存即被清除，届时相关缓存数据将无法被访问。
...
为提升速度，本服务可能经由境外网络加速节点传输部分请求或静态资源；我们不在境外持久化存储涉及个人数据，境外节点仅承担网络层级的加速与转发，不进行任何数据内容处理或存储，所有实际数据处理行为均在境内服务器完成。

3.6 本服务排行榜功能所使用的用户标识号通过用户凭证使用单向哈希算法（SHA-256 + 盐值）生成，仅用于区分不同用户，不包含可识别的个人信息，不可逆推出原始标识。
...
除您上传的合法内容与5.2，5.3所述的第三方权利内容外，本服务的全部权利均归开发者所有。您保留您在使用本服务过程中上传的内容（如玩家成绩等）的合法权利，但您同意授予开发者在服务运行、展示及优化中为实现相关功能所必需的非独占的、全球范围的、免许可费的使用权。您可通过本协议第 10 条联系方式撤回上述授权，但撤回不影响撤回前已基于授权完成的使用。撤回不影响已完成的合法、非商业性使用；对未经许可的商业使用，开发者保留追责权利。
...
"生成内容"对这些素材的引用，旨在为用户提供便利，以实现对相关作品的介绍、评论或说明等目的下的**适当引用**。**这不代表获得了任何权利人的任何官方授权，也不代表相关权利人对本服务的认可、承诺或任何形式的背书**。
...
5.5 我们尊重并保护权利人的知识产权。如果您是相关知识产权的合法权利人，并且认为本服务的“生成内容”侵犯了您的合法权益，请您通过本协议第10条提供的联系方式，向我们发送书面通知。您的通知应包含以下内容：
...
**您理解并同意，除非因开发者故意或重大过失造成的损害，否则即使本协议中的某些条款因故无效，开发者对您承担的全部赔偿责任，累计总额不应超过用户为使用本服务所向开发者支付的费用**。
...
6.5 开发者仅提供服务，无法也无义务审查用户对“生成内容”的后续使用行为。**您理解并同意，如果任何第三方权利人对“生成内容”中的素材提出权利主张或异议，我们有权在收到有效权利通知后，采取包括但不限于移除相关内容、限制功能或单方面终止本协议的措施。**。
...
**我们会采取合理措施公布协议更改情况，如您不接受修改后的协议，请您立即停止使用本服务**。您继续使用本服务将被视为您已充分阅读、理解并接受更新后的本协议并愿意受更新后的本协议的约束。

我们建议您定期访问本页面以查阅最新版本。

7.2 开发者保留随时因任何理由（包括但不限于收到相关权利方（如Pigeon Games）的通知、技术、运营成本、法律法规要求或开发者个人决定）**随时中止或终止本服务的权利。除法律另有规定外，开发者不承担因此产生的赔偿责任**。

7.3 我们可通过页面公告、弹窗、电子邮件或其他合理方式向您发出通知。上述方式一经发送或发布，即视为有效送达。若用户在公告后继续使用本服务，即视为已知悉并同意变更内容。
