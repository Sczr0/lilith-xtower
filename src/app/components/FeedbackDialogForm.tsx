...
export interface FeedbackDialogFormProps {
  /** 受控开关；由 FeedbackDialog 的外壳持有，便于把 Radix 依赖隔离到懒加载 chunk。 */
  open: boolean;
...
/**
 * 反馈弹窗的「重」部分：静态依赖 @radix-ui/react-dialog。
 *
 * 从 FeedbackDialog 抽出并懒加载（首次打开才下载），避免 Radix Dialog 进入
 * 全站每个带页脚页面的首屏包。
 *
 * 注意：可能被错误页渲染（此时 AuthProvider 可能已损坏），
 * 因此登录态用“可选”方式读取，缺 Provider 时按未登录处理。
 */
export function FeedbackDialogForm({ open, onOpenChange, initialError }: FeedbackDialogFormProps) {
  ...
  // 每次打开时的准备（原 handleOpenChange 中 open=true 分支的逻辑）。
  useEffect(() => {
    if (!open) return;
    // 打开时重置上一次的提交结果/诊断面板，属刻意的同步初始化
    // eslint-disable-next-line react-hooks/set-state-in-effect -- 打开时的一次性重置
    setResult(null);
