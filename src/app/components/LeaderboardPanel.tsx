  ...
  return (
    <div className="space-y-8">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white/80 p-6 shadow-sm shadow-gray-100/40 backdrop-blur-sm transition-colors dark:border-neutral-800 dark:bg-neutral-900/50 dark:shadow-none">
          ...
          >
            ...
            <input
              ...
              placeholder="输入新的别名"
              autoComplete="off"
              className={inputStyles({ className: 'flex-1 min-w-0' })}
            />
            <button
              type="submit"
              disabled={isAliasUpdating || aliasInput.trim().length === 0 || !isAliasDirty}
              className={getButtonStyles({ variant: 'secondary', className: 'flex-shrink-0 whitespace-nowrap' })}
            >
              {isAliasUpdating ? (
                <>
                  <svg className="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24" role="img">
      ...
      <LeaderboardTopTableSection
        ...
        topError={topError}
        topPageSize={topPageSize}
        topPageSizeOptions={TOP_LIMIT_SELECT_OPTIONS}
        canLoadMore={canLoadMore}
        copyHint={copyHint}
        onTopPageSizeChange={handleTopPageSizeChange}
        onRefreshTop={handleRefreshTop}
        onLoadMore={handleLoadMore}
        onLookupAlias={handleLookupFromLeaderboard}
        onCopyUser={(user) => {
          void copyText(user, '用户标识');
        }}
