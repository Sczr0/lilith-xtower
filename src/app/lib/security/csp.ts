export function buildContentSecurityPolicy() {
  ...
  const capUpstream = (process.env.CAP_PUBLIC_URL || '').trim() || 'https://cap.xtower.site';

  return [
    "default-src 'self'",
    `script-src 'self' 'unsafe-inline' https://cloud.umami.is ${capCdn}`.trim(),
    `connect-src 'self' https://cloud.umami.is https://cloudflareinsights.com https://api-gateway.umami.dev https://api.umami.is https://seekend.xtower.site https://accounts.tapapis.com https://accounts.tapapis.cn https://afdian.com https://pic1.afdiancdn.com https://somnia.xtower.site https://rjsperf.myalicdn.com ${capUpstream} ${capCdn}`.trim(),
